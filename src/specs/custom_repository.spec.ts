...
import { pagination, repositories, RequestContext, serializers } from '..';
...
describe('Full Case', () => {
  ...
  class UserRepository extends repositories.APIRepository {
    ...
    // @ts-ignore
    public async get(config: Partial<RequestContext> = {}) {
  ...
  it('Custom Repository should call update properly', async () => {
    ...
    assert<Equals<GetReturnType, User>>();
    expect(spyClient).toBeCalledWith('/api/users/me/', {
      first_name: 'Anton',
      last_name: 'Shuvalov',
      phone: '+840901234567',
    });
