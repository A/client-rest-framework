...
// TODO: write filter like for undefined, but which makes type | null partial
type PartialOptional<T> = {
  [P in keyof T]-?: PartialOptional<NonNullable<T[P]>>;
};
...
type SerializedDTOResult<Serializer extends Record<string, any>> =
  NoUndefinedField<{
    [Key in KnownKeys<Serializer>]:
    Serializer[Key]['readonly'] extends true
    ? undefined
